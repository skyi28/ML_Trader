...
class Database:
    ...
    def provide_uniue_user_id(self) -> int:
        """
        This function generates a unique user ID by fetching the highest existing ID from the 'user' table in the PostgreSQL database.
        It then increments the highest ID by 1 to ensure the new ID is unique.

        Parameters:
        - self (Database): The instance of the Database class. This parameter is used to access the execute_read_query method.

        Returns:
        - int: The unique user ID generated by incrementing the highest existing ID by 1.
        """
        new_id = self.execute_read_query(f'SELECT "id" FROM "user" ORDER BY "id" DESC LIMIT(1)', first_only=True)[0] + 1
        return new_id
